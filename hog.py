...
def boar_brawl(player_score, opponent_score):
    ...
    # 计算绝对差值
    abs_diff = abs(opponent_tens_digit - player_ones_digit)
    # 绝对差值*3 与 1 取较大值
    return max(abs_diff * 3, 1)
    # END PROBLEM 2
...
def play(strategy0, strategy1, update,
    ...
    while score0 < goal and score1 < goal:
        ...
        if score0 >= goal or score1 >= goal:
            break
        who = 1 - who  # switch between players
    # END PROBLEM 5
...
# 从代码中，我们可以看到 `roll_dice` 函数模拟掷骰子 `num_rolls` 次。它将所有掷出的结果相加，除非其中有一个结果是 1。如果有掷出的结果是 1，那么函数将返回 1。

# 在这段代码中，我们有一个名为 `sow_sad` 的布尔变量，用于跟踪是否有掷出的结果为 1。在循环中，我们将每次掷出的结果累加到 `total` 变量中。如果掷出的结果是 1，我们将 `sow_sad` 设置为 `True`。在循环结束后，如果 `sow_sad` 为 `True`，则返回 1，否则返回 `total`。

# 结合之前的示例，我们可以看到 `dice` 函数在掷出 1 的情况下会导致 `sow_sad` 为 `True`。因此，当我们调用 `averaged_roll_dice(2, dice)` 时，`roll_dice` 函数将返回以下结果：
# 1. 第一次调用：掷出 3 和 1，结果为 1（因为有一个 1）
# 2. 第二次调用：掷出 5 和 6，结果为 11
# 3. 第三次调用：掷出 3 和 1，结果为 1（因为有一个 1）
# 4. 第四次调用：掷出 5 和 6，结果为 11
# 5. 以此类推...

# 在这个示例中，我们重复了 1000 次。由于 `dice` 的顺序是固定的，我们可以预期 `roll_dice` 的结果将在 1 和 11 之间交替。因此，计算平均值时，我们可以预期结果接近 (1 + 11) / 2 = 6。

    first_output = None
    for score0 in range(goal):
        for score1 in range(goal):
            current_output = strategy(score0, score1)
            if first_output is None:
                first_output = current_output
            elif first_output != current_output:
                return False
    return True
    "*** YOUR CODE HERE ***"
    # END PROBLEM 7
